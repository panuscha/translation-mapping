...
try:
    ...
    geotagged_df['region_country_name'] = country_names
    geotagged_df.to_csv("geotagged/geotagged_germany_country.csv")

    flattened_data = [(key[0], key[1], key[2], value) for key, value in not_found_cities.items()]
    
    with open('cities_not_found_new.csv', 'w', newline='', encoding = 'utf8') as file:
